...
trait HasNutCoreParameter {
  val XLEN                = if (Settings.get("IsRV32")) 32 else 64 // 机器字长
  val HasMExtension       = true // M 拓展 mul div
  val HasDiv              = true
  val HasCExtension       = Settings.get("EnableRVC") // C 拓展 压缩指令
  val HasIcache           = Settings.get("HasIcache")
  val HasDcache           = Settings.get("HasDcache")
  val HasITLB             = Settings.get("HasITLB") // instr tlb
  val HasDTLB             = Settings.get("HasDTLB")
  val AddrBits            = 64 // AddrBits is used in some cases
  val VAddrBits           = if (Settings.get("IsRV32")) 32 else 39 // sv39 虚拟地址是 39 bit 的
  val PAddrBits           = 32 // 应该是只有 4G 的内存
  val AddrBytes           = AddrBits >> 3 // unused
  val DataBits            = XLEN
  val DataBytes           = DataBits >> 3
  val EnableVirtualMemory = if (Settings.get("HasDTLB") && Settings.get("HasITLB")) true else false
  val EnablePerfCnt       = true // 开启 prefetch
  // Parameter for Argo's OoO backend
  val EnableMultiIssue          = Settings.get("EnableMultiIssue") // 开启多发射
  val EnableOutOfOrderExec      = Settings.get("EnableOutOfOrderExec") // 开启乱序执行
  val EnableMultiCyclePredictor = false // 多周期预测
  val EnableOutOfOrderMemAccess = false // 乱序内存访问
}

trait HasNutCoreConst extends HasNutCoreParameter {
  val CacheReadWidth        = 8 // 定义 cache 读取的 width
...
class NutCore(implicit val p: NutCoreConfig) extends NutCoreModule {
  ...
  if (EnableOutOfOrderExec) {
    ...
    if (EnableVirtualMemory) {
      ...
      io.dmem <> Cache(in = backend.io.dmem, mmio = mmioXbar.io.in.drop(1), flush = "b00".U, empty = dtlb.io.cacheEmpty, enable = HasDcache)(CacheConfig(ro = false, name = "dcache", userBits = DCacheUserBundleWidth))
    }
