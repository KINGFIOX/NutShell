...
class SimpleBusCrossbar1toN(addressSpace: List[(Long, Long)]) extends Module {
  ...
  for (i <- 0 until io.out.length) {
    io.out(i).resp.ready := outSelRespVec(
      i
    ) && io.in.resp.ready && state === s_resp
  }
  io.in.resp.valid := Mux1H(
    outSelRespVec,
    io.out.map(_.resp.valid)
  ) || state === s_error
  ...
  Debug() {
    when(io.in.req.fire) {
      printf(
        p"${GTimer()}: xbar: outSelVec = ${outSelVec}, outSel.req: ${io.in.req.bits}\n"
      )
    }
    when(io.in.resp.fire) {
      printf(
        p"${GTimer()}: xbar: outSelVec = ${outSelVec}, outSel.resp: ${io.in.resp.bits}\n"
      )
    }
  }
}
    ...
    is(s_idle) {
      when(thisReq.fire) {
        ...
        when(thisReq.bits.isRead()) { state := s_readResp }
          .elsewhen(
            thisReq.bits.isWriteLast() || thisReq.bits.isWriteSingle()
          ) { state := s_writeResp }
      }
    }
    is(s_readResp) {
      when(io.out.resp.fire && io.out.resp.bits.isReadLast) { state := s_idle }
    }
    is(s_writeResp) { when(io.out.resp.fire) { state := s_idle } }
  }
}

class SimpleBusCrossbar(n: Int, addressSpace: List[(Long, Long)])
    extends Module {
  val io = IO(new Bundle {
...
class SimpleBusAutoIDCrossbarNto1(n: Int, userBits: Int = 0) extends Module {
  ...
  Debug() {
    when(io.out.req.fire) {
      printf(
        "[Crossbar REQ] addr %x cmd %x select %b\n",
    ...
    when(io.out.resp.fire) {
      printf(
        "[Crossbar RESP] data %x select %b\n",
        io.out.resp.bits.rdata,
        io.out.resp.bits.id.get
      )
    }
  }
}
