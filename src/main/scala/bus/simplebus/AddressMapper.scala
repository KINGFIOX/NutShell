...
/** @param map
  *   -> (regionBits, rebase) @regionBits: 地址映射的位数 @rebase: 地址映射的基地址（ 放到高位 ）
  */
class SimpleBusAddressMapper(map: (Int, Long))
    extends Module
  ...
  io.out <> io.in // default

  val (regionBits, rebase) = (map._1, map._2.U(PAddrBits.W))
  if (regionBits != 0) {
    io.out.req.bits.addr := Cat(
      rebase(PAddrBits - 1, regionBits), // PAddrBits 是 paddr 的宽度
      io.in.req.bits.addr(regionBits - 1, 0)
    )
  }
}
